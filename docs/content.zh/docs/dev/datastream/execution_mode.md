...
一些特殊情况下，你可以使用`流`模式运行有边界。一个例外情况为当你想使用该有边界作业的运行结果去初始化一些作业状态，并将状态在之后的无边界作业中使用。例如，通过`流`模式运行一个有边界作业，获取一个 savepoint，然后在一个无边界作业上恢复这个 savepoint。目前来说这是一个可行但非常特殊的用例。当我们允许将 savepoint 作为 `批` 执行作业的附加输出时，这个用例会被以`批`执行模式运行有边界作业的更好实践所取代。
...
将多个操作/算子链接在一起的功能称为[链（Chaining）]({{< ref "docs/dev/datastream/operators/overview" >}}#task-chaining-and-resource-groups)。Flink 称一个调度单位的一组或多个（链接在一起的）算子为一个 _任务_。通常，_子任务_ 用来指代在多个 TaskManager 上并行运行的单个任务实例，但我们在这里只使用 _任务_(task)一词。
